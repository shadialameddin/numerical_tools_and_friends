...
#define MEXPLUS_ASSERT(condition, ...)                                         \
  if (!(condition))                                                            \
  mexErrMsgIdAndTxt("mexplus:error", __VA_ARGS__)
...
public:
  ...
  explicit MxArray(const mxArray *array)
      : array_(const_cast<mxArray *>(array)), owner_(false) {}
   ...
   */
  template <typename T> static mxArray *Numeric(int rows = 1, int columns = 1);
  /** Create a new numeric (real or complex) matrix.
   * @param ndim Number of dimensions.
   * @param dims Dimensions array. Each element in the dimensions array
   *             contains the size of the array in that dimension.
   */
  template <typename T> static mxArray *Numeric(std::vector<std::size_t> dims);
   ...
   * @endcode
   */
  static mxArray *Struct(int nfields = 0, const char **fields = NULL,
                         int rows = 1, int columns = 1) {
    mxArray *struct_array =
        mxCreateStructMatrix(rows, columns, nfields, fields);
    MEXPLUS_CHECK_NOTNULL(struct_array);
    return struct_array;
  }
  /** mxArray* importer methods.
   */
  template <typename T> static mxArray *from(const T &value) {
    return fromInternal<T>(value);
  }
   ...
   */
  template <typename T> static void to(const mxArray *array, T *value) {
    toInternal<T>(array, value);
  }
  template <typename T> static T to(const mxArray *array) {
    T value;
    toInternal<T>(array, &value);
    return value;
  }
   ...
   */
  template <typename T> static T at(const mxArray *array, mwIndex index) {
    T value;
  ...
  template <typename T>
  static void at(const mxArray *array, const std::string &field, T *value,
                 mwIndex index = 0) {
    atInternal<T>(array, field, index, value);
  }
  static const mxArray *at(const mxArray *array, const std::string &field,
                           mwIndex index = 0) {
    MEXPLUS_CHECK_NOTNULL(array);
  ...
  static void set(mxArray *array, mwIndex index, mxArray *value) {
    ...
    MEXPLUS_ASSERT(static_cast<size_t>(index) < mxGetNumberOfElements(array),
                   "Index out of range: %u.", index);
    mxDestroyArray(mxGetCell(array, index));
    mxSetCell(array, index, value);
  }
  template <typename T>
  static void set(mxArray *array, const std::string &field, const T &value,
                  mwIndex index = 0) {
    setInternal<T>(array, field, index, value);
  }
  static void set(mxArray *array, const std::string &field, mxArray *value,
                  mwIndex index = 0) {
    MEXPLUS_CHECK_NOTNULL(array);
    MEXPLUS_CHECK_NOTNULL(value);
    MEXPLUS_ASSERT(mxIsStruct(array), "Expected a struct array.");
    ...
    if (field_number < 0) {
      ...
      MEXPLUS_ASSERT(field_number >= 0, "Failed to create a field '%s'",
                     field.c_str());
    }
  ...
  template <typename T> void at(mwIndex index, T *value) const {
    atInternal<T>(array_, index, value);
  }
  const mxArray *at(mwIndex index) const { return at(array_, index); }
  /** Template for element accessor.
   ...
   * @return pointer T*. If MxArray is not compatible, return NULL.
   */
  template <typename T> T *getImagData() const;
  mxLogical *getLogicals() const {
    MEXPLUS_CHECK_NOTNULL(array_);
    MEXPLUS_ASSERT(isLogical(), "Expected a logical array but %s.",
                   className().c_str());
    return mxGetLogicals(array_);
  }
  mxChar *getChars() const {
    MEXPLUS_CHECK_NOTNULL(array_);
    MEXPLUS_ASSERT(isChar(), "Expected a char array but %s.",
                   className().c_str());
  ...
  mwIndex subscriptIndex(const std::vector<mwIndex> &subscripts) const {
    return mxCalcSingleSubscript(array_, static_cast<mwSize>(subscripts.size()),
  ...
  bool hasField(const std::string &field_name, mwIndex index = 0) const {
    return isStruct() && mxGetField(array_, index, field_name.c_str()) != NULL;
  }
  /** Element size.
   */
  int elementSize() const { return static_cast<int>(mxGetElementSize(array_)); }
...
private:
  ...
  template <typename T>
  static mxArray *
  fromInternal(const typename std::enable_if<MxArithmeticType<T>::value,
                                             T>::type &value);
  /** Complex types, complex<float> or complex<double>.
   */
  template <typename T>
  static mxArray *fromInternal(
  ...
  static mxArray *fromInternal(
      const typename std::enable_if<MxArithmeticCompound<Container>::value,
                                    Container>::type &value);
  /** Container with complex numbers, i.e. vector<complex<double>>.
   */
  template <typename Container>
  static mxArray *fromInternal(
      const typename std::enable_if<MxComplexCompound<Container>::value,
                                    Container>::type &value);
  /** Char type */
  template <typename T>
  static mxArray *fromInternal(
      const typename std::enable_if<MxCharType<T>::value, T>::type &value);
  /** Containter with signed char.
   */
  template <typename Container>
  static mxArray *
  fromInternal(const typename std::enable_if<
  ...
  template <typename T>
  static mxArray *fromInternal(
      const typename std::enable_if<MxLogicalType<T>::value, T>::type &value);
  /** Container with logicals.
   */
  template <typename Container>
  static mxArray *fromInternal(
      const typename std::enable_if<MxLogicalCompound<Container>::value,
                                    Container>::type &value);
  /** Container with cell type content.
   */
  template <typename Container>
  static mxArray *
  fromInternal(const typename std::enable_if<MxCellCompound<Container>::value,
                                             Container>::type &value);
  ...
  template <typename T>
  static void
  toInternal(const mxArray *array,
             typename std::enable_if<
                 MxArithmeticType<T>::value || MxComplexType<T>::value ||
                     MxLogicalType<T>::value || MxCharType<T>::value,
                 T>::type *value) {
  ...
  static void
  toInternal(const mxArray *array,
             typename std::enable_if<MxComplexOrArithmeticCompound<T>::value ||
   ...
   */
  template <typename T>
  static void toInternal(
      const mxArray *array,
      typename std::enable_if<MxCellType<T>::value &&
  ...
  template <typename T>
  static void
  atInternal(const mxArray *array, mwIndex index,
             typename std::enable_if<MxComplexOrArithmeticType<T>::value ||
  ...
  static void atInternal(const mxArray *array, mwIndex index,
                         typename std::enable_if<std::is_compound<T>::value &&
  ...
  template <typename T>
  static void
  setInternal(mxArray *array, mwIndex index,
              const typename std::enable_if<!std::is_compound<T>::value ||
                                                MxComplexType<T>::value,
                                            T>::type &value);
  /** Container types.
   */
...
#pragma warning(disable : 4244)
  ...
  static void assignTo(const mxArray *array, mwIndex index,
                       typename std::enable_if<std::is_floating_point<R>::value,
                                               R>::type *value) {
...
#pragma warning(pop)
  ...
  template <typename T, typename R>
  static void
  assignTo(const mxArray *array, mwIndex index,
...
#pragma warning(disable : 4244 4800)
  ...
  template <typename T, typename R>
  static void
  assignTo(const mxArray *array,
           typename std::enable_if<MxArithmeticCompound<R>::value ||
    ...
    if (!mxIsComplex(array)) {
      ...
      for (mwSize i = 0; i < array_size; ++i) {
        double mag =
            std::abs(std::complex<double>(static_cast<double>(*(real_part++)),
...
#pragma warning(pop)
   ...
   */
  template <typename R>
  static void assignStringTo(
      const mxArray *array,
      typename std::enable_if<std::is_signed<typename R::value_type>::value,
                              R>::type *value) {
    typedef typename std::make_signed<mxChar>::type SignedMxChar;
    SignedMxChar *data_pointer =
        reinterpret_cast<SignedMxChar *>(mxGetChars(array));
...
#pragma warning(disable : 4244 4800)
  ...
  static void assignFrom(
      ...
      const typename std::enable_if<
          MxArithmeticType<T>::value || MxCharType<T>::value, T>::type &value) {
    if (mxIsComplex(array)) {
...
#pragma warning(disable : 4244)
  ...
  template <typename T>
  static void
  assignCharFrom(mxArray *array, mwIndex index,
                 const typename std::enable_if<std::is_floating_point<T>::value,
                                               T>::type &value) {
...
#pragma warning(pop)
  ...
  assignCharFrom(mxArray *array, mwIndex index,
    ...
    *(mxGetChars(array) + index) =
        reinterpret_cast<const typename std::make_unsigned<T>::type &>(value);
  }
  template <typename T>
  static void
  assignCharFrom(mxArray *array, mwIndex index,
 ...
 */
template <typename T>
mxArray *MxArray::fromInternal(
    const typename std::enable_if<MxArithmeticType<T>::value, T>::type &value) {
  mxArray *array =
      mxCreateNumericMatrix(1, 1, MxTypes<T>::class_id, MxTypes<T>::complexity);
  MEXPLUS_CHECK_NOTNULL(array);
...
mxArray *MxArray::fromInternal(
  ...
  mxArray *array = mxCreateNumericMatrix(1, static_cast<int>(value.size()),
...
mxArray *MxArray::fromInternal(
  ...
  mxArray *array =
      mxCreateNumericMatrix(1, static_cast<int>(value.size()),
                            MxTypes<ContainerValueType>::class_id, mxCOMPLEX);
  MEXPLUS_CHECK_NOTNULL(array);
...
mxArray *MxArray::fromInternal(
  ...
  for (typename Container::const_iterator it = value.begin(); it != value.end();
       ++it) {
...
template <typename Container>
mxArray *MxArray::fromInternal(
    const typename std::enable_if<MxCellCompound<Container>::value,
                                  Container>::type &value) {
...
template <typename T>
void MxArray::atInternal(const mxArray *array, const std::string &field,
                         mwIndex index, T *value) {
  MEXPLUS_CHECK_NOTNULL(array);
  MEXPLUS_CHECK_NOTNULL(value);
  MEXPLUS_ASSERT(static_cast<size_t>(index) < mxGetNumberOfElements(array),
...
template <typename T>
void MxArray::setInternal(mxArray *array, const std::string &field,
                          mwIndex index, const T &value) {
  MEXPLUS_CHECK_NOTNULL(array);
  MEXPLUS_ASSERT(static_cast<size_t>(index) < mxGetNumberOfElements(array),
                 "Index out of range: %u.", index);
...
template <typename T> mxArray *MxArray::Numeric(int rows, int columns) {
  typedef typename std::enable_if<MxComplexOrArithmeticType<T>::value, T>::type
      Scalar;
  mxArray *numeric = mxCreateNumericMatrix(
      rows, columns, MxTypes<Scalar>::class_id, MxTypes<Scalar>::complexity);
  MEXPLUS_CHECK_NOTNULL(numeric);
  return numeric;
}

template <typename T> mxArray *MxArray::Numeric(std::vector<std::size_t> dims) {
  typedef typename std::enable_if<MxComplexOrArithmeticType<T>::value, T>::type
      Scalar;
  mxArray *numeric =
      mxCreateNumericArray(dims.size(), &dims[0], MxTypes<Scalar>::class_id,
                           MxTypes<Scalar>::complexity);
  MEXPLUS_CHECK_NOTNULL(numeric);
  return numeric;
}

template <typename T> T *MxArray::getData() const {
  MEXPLUS_CHECK_NOTNULL(array_);
  MEXPLUS_ASSERT(MxTypes<T>::class_id == classID(), "Expected a %s array.",
                 typeid(T).name());
  return reinterpret_cast<T *>(mxGetData(array_));
}

template <typename T> T *MxArray::getImagData() const {
  MEXPLUS_CHECK_NOTNULL(array_);
  MEXPLUS_ASSERT(MxTypes<T>::class_id == classID(), "Expected a %s array.",
                 typeid(T).name());
...
template <typename T> T MxArray::at(mwIndex row, mwIndex column) const {
