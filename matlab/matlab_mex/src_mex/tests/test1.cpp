...
#define dimension   int(*mxGetPr(prhs[5]))
...
void mexFunction(int nlhs, mxArray* plhs[],
    ...
    // std::cout << typeid(pDims).name() << '\n';
     ...
     Matrix3 sigma_i_j=Matrix::Zero(3,3); // Aa.setZero(3,3);
     Matrix3 eps_e_i_j=Matrix::Zero(3,3); // Aa.setZero(3,3);
     Vector3 x;
     Matrix3 v;
     Matrix3 sigma_pos;
     ...
     for(int i=0; i<ngp; i++)
     {
         for(int j=0; j<ntp; j++)
         {
            ...
            sigma_i_j << sigma[j*ngp*ncomp+i*ncomp+0],             inv_sqrt2 * sigma[j*ngp*ncomp+i*ncomp+5], inv_sqrt2 * sigma[j*ngp*ncomp+i*ncomp+4],
                         ...
                         inv_sqrt2 * sigma[j*ngp*ncomp+i*ncomp+4], inv_sqrt2 * sigma[j*ngp*ncomp+i*ncomp+3], inv_sqrt2 * sigma[j*ngp*ncomp+i*ncomp+2];
...
//             std::cout<< sigma_pos <<std::endl<<std::endl;
//             std::cout<< sigma_neg <<std::endl<<std::endl;
