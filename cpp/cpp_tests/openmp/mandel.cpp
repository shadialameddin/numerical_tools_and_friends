...
#pragma omp parallel for collapse(2) private(c) firstprivate(eps) reduction(+ : numoutside)
  for (i = 0; i < NPOINTS; i++) {
    for (j = 0; j < NPOINTS; j++) {
      c.r = -2.0 + 2.5 * (double)(i) / (double)(NPOINTS) + eps;
      c.i = 1.125 * (double)(j) / (double)(NPOINTS) + eps;
      { numoutside = testpoint(c, numoutside); }
    }
  }
...
int testpoint(d_complex c, int numoutside) {
