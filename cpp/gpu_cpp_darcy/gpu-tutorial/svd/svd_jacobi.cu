...
int main(int argc, char* argv[])
{
    ...
    gpu_matrix d_A_test(rows_A, cols_A);

    check(cudaMemcpy(d_A_test.data(),
                     A.data(),
                     sizeof(double) * rows_A * cols_A,
                     cudaMemcpyHostToDevice));

    std::cout << "/* message */" << d_A_test.rows() << '\n';
    std::cout << "/* message */" << d_A_test.columns() << '\n';
    std::cout << "/* ------------------------------------------- */" << '\n';
    ...
    // void gpu_matrix_multiplication(cublasH, A, false, B, false, C, 1.0, 0.0);
