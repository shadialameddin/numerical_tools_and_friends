...
void load_board(std::vector<char>& board,
                const char* file,
...
int main(int argc, char** argv)
{
    ...
    try
    {
        ...
        cl::make_kernel<cl::Buffer, cl::Buffer, unsigned int, unsigned int, cl::LocalSpaceArg>
        ...
        for (unsigned int i = 0; i < iterations; i++)
        {
            ...
            // Enqueue the kernel
            accelerate_life(cl::EnqueueArgs(queue, global, local),
                            d_board_tick,
                            d_board_tock,
                            nx,
                            ny,
...
void load_params(const char* file, unsigned int* nx, unsigned int* ny, unsigned int* iterations)
{
    ...
    if (!fp.is_open()) die("Could not open params file.", __LINE__, __FILE__);
...
{
    std::ifstream fp(file);
    if (!fp.is_open()) die("Could not open input file.", __LINE__, __FILE__);
